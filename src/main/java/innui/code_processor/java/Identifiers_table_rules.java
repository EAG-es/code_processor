...
public class Identifiers_table_rules extends innui.code_processor.Identifiers_table_rules {
    ...
    public void load(Oks ok, Object ... extras_array) throws Exception {
        ...
        try {
            ...
            Analizer_rules.Tokens_or_rule_nodes package_tokens_or_rule_node = ok.valid(define_rule_package(ok, extras_array));
            if (ok.is == false) return;
    ...
    public Analizer_rules.@Nullable Tokens_or_rule_nodes define_rule_package(Oks ok, Object ... extras_array) throws Exception {
        ...
        try {
            ...
            ok.valid(package_1_tokens_or_rule_node.defined_first_part_evaluation_tokens_list).add(new Scanner_rules.Basic_tokens(identifier.name()));
            ...
            ok.valid(package_2_tokens_or_rule_node.defined_first_part_evaluation_tokens_list).add(new Scanner_rules.Basic_tokens(dot.name()));
            ...
            ok.valid(package_3_tokens_or_rule_node.defined_first_part_evaluation_tokens_list).add(new Scanner_rules.Basic_tokens(identifier.name()));
     ...
     * @throws Exception
     */
    public @Nullable Integer process(Scanner_rules.Basic_tokens basic_token, Oks ok, Object ... extras_array) throws Exception {
        new Test_methods(ok, ok, extras_array, this);
        if (ok.is == false) return null;
        Integer retorno = null;
        try {
            boolean is;
            is = filter_token(basic_token, ok, extras_array);
            if (ok.is == false) return null;
            if (is) {
                return null;
            }
            is = count_braces(basic_token, ok, extras_array);
            if (ok.is == false) return null;
            if (is) {
                return null;
            }
            ok.valid(ok.valid(analizer_rules).start_rule_node).evaluate(basic_token, ok, extras_array);
